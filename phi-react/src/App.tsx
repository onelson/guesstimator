...
function App() {
  ...
  return (
      <div className="container mx-auto flex flex-col space-y-4">
        ...
        <CardPicker/>
        {isAdmin ? (<button className="btn-red" onClick={handleCall}>Call</button>) : null}
      </div>
  );
...
function PlayerCards() {
    ...
    const players: Player[] = [
        { id: "xxxx", selectedCard: 1, name: "Trent"}
    ];  // FIXME
          ...
          {players.map((player) => {
              return (<div key={player.id}>
                  <div className={`card ${player.selectedCard ?  "" : "undecided"}`}>
                      <div className={"value"}>{player.selectedCard ? cards[player.selectedCard] : ""}</div>
                  </div>
